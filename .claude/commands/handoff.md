...
- Don't be vague ("fix the bug") — be specific ("fix the reconnection in `src/hooks/useSSE.ts` by adding exponential backoff after the `onclose` handler")
...
- **Uncommitted changes?** → Run `/commit` first to save your work
- **Ready to end session?** → Share the handoff path with the user
- **Starting the next session?** → Open with: `Read HANDOFF.md and continue from where the previous session left off.`
- **Handoff no longer needed?** → Delete `HANDOFF.md` after the next session picks it up
